# Usage:
# python VIT_model.py --job F5_rep1 --Landmark F_5 
...
from tensorflow.keras.layers import Input, Conv2D, MaxPooling2D, Dropout, concatenate, UpSampling2D, Conv2DTranspose, Flatten, Activation, Dense
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Launch VIT model ! Landmark specific models")
    ...
    # MODEL ARCHITECTURE
    #------------------------
    vit_model = vit.vit_b32(image_size=224, activation='relu', include_top=True, pretrained=True)
    ...
    # Layers for landmarks predictions
    ...
    model.fit(train_datagen.flow(X_train_images, X_train_lds_uv_coords, batch_size=128), 
              epochs=1000,
              callbacks=[early_stopping, csv_logger, checkpoint, lr_scheduler],
