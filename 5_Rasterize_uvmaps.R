...
dir.create(file.path(path2raster), showWarnings=FALSE)

#for (file_id in filenames_param_communs[1:5]){
...
#   print(head(luminance_tab))
# Extract texture information from uvmapped parameterization files 
   OBJ_param_lscm <- readLines(con  = paste0(path_param_files, gsub(".obj", "", file_id), "_lscm.obj"))
...
furrr::future_map(filenames_param_communs, rasterization)
#plan(sequential)
